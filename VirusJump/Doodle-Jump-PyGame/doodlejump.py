...
pygame.display.set_caption("Virus Jump!")
...
def gameLoop():
    ...
    while not gameExit:
        ...
        while gameOver==True:
            gameDisplay.fill(white)
            messageToScreen("Вы проиграли! Нажмите A чтобы сыграть еще раз. Нажмите Q чтобы выйти!",red,display_width/2-220,display_height/2)
            pygame.display.update()
       ...
       #pygame.draw.rect(gameDisplay,white,[lead_x,lead_y,block_width,block_height])
        messageToScreen("Welcome to Virus JUMP",red,display_width/2-100,display_height - 50)
        pygame.display.update()
        clock.tick(fps)
